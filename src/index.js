...
function changeItemInfoDueToColor(userQTY) {
  ...
  item_title.innerHTML = `<h1>Plush Toy ${
    colorSelection.charAt(0).toUpperCase() + colorSelection.slice(1)
  }</h1>`;
...
cycleBtn.addEventListener("click", function (event) {
  let currentColor = setImageColor(
    color_dropdown[color_dropdown.selectedIndex].value
  );
  currentImgIndex += 1;
